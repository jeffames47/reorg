...
#export dist_list=jeffrey.ames@navihealth.com,Luis.Arce@navihealth.com,Chad.Binkley@navihealth.com
#export dist_list=jeffrey.ames@navihealth.com,Chad.Binkley@navihealth.com.Bruce.Jackson@navihealth.com
...
#  dist_list_with_benny=jeffrey.ames@navihealth.com,Chad.Binkley@navihealth.com,Bruce.Jackson@navihealth.com
#  dist_list=jeffrey.ames@navihealth.com,Chad.Binkley@navihealth.com,Bruce.Jackson@navihealth.com
dist_list_with_benny=jeffrey.ames@wellsky.com
...
if [ ${B4_OR_AFTER} == AF ]; then
    ...
    binfo_AF=`cat /etc/redhat-release ; uname -r`
    if [[ ! -z ${patch_info_diff} ]]
    then
        if [[ ${debug} == d ]]
        then
            echo -e "\n OS Before patch: $binfo_B4 "
            echo -e " OS after patch: $binfo_AF     ....The patch was.... successful"
        fi
           echo -e "\n Patch for server: $hst \n OS Before patch: $binfo_B4 \n ### \n  OS after patch: $binfo_AF  \n\n Server has been up since `uptime`.  The patch was most likely successful! \n OS level is `uname -r` " | /bin/mail -s " Server: $hst patch successful " $dist_list
    else
        if [[ ${debug} == d ]]
        then
           echo -e "\n OS Before patch: $binfo_B4 "
           echo -e " OS after patch: $binfo_AF     ....The patch was....    NOT SUCCESSFUL"
        fi
           echo -e "\n Patch for server: $hst \n OS Before patch: $binfo_B4 \n ###### ################# ######### \n OS after patch: $binfo_AF  \n\n Before and after files are the same.  The patch was perhaps NOT successful!! \n OS level is `uname -r` " | /bin/mail -s " 'WARNING' Server: $hst The patch was NOT successful " $dist_list
    fi
